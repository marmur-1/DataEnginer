...
def write_ticker_to_db(path_file="tickets.json"):
    ...
    for Timestamp in raw_data["Time Series (1min)"]:
        ...
        data.append((Symbol, Timestamp, Open, Close, High, Low, Volume))
        
    print(data)
    args = ','.join(cursor.mogrify("(%s,%s,%s,%s,%s,%s,%s)", i).decode('utf-8') for i in data)
    insert_query = "INSERT INTO tickets (symbol, timestamp, open, close, high, low, volume) VALUES " + (args) + " ON CONFLICT (symbol, timestamp) DO NOTHING;"
