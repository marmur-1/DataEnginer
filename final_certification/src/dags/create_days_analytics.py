...
def create_days_analytics(id_day):
    ...
    query = "SELECT count(*) FROM tickets WHERE id_day = %s;"
    cursor.execute(query,[id_day])
    results = cursor.fetchone()
    if results[0] > 0:
        ...
        min_price_interval = datetime.utcfromtimestamp(int(df.loc[df['low'] == df.low.min()].sort_values(by='timestamp').timestamp.values[0])/1e9)
